...
const TableOfContents: React.FC<TableOfContentsProps> = ({ sections }) => {
  ...
  return (
    <div>
      <button
        className="md:hidden fixed bottom-1/4 right-4 transform translate-y-1/2 z-50 p-2 bg-highlight-purple text-white rounded"
                  ...
                  <a
                    ...
                    className={`text-sm text-dark-grey px-2 py-1 rounded transition-transform transition-colors duration-300 ease-in-out ${
                      activeSection === section
                        ? "text-highlight-purple scale-105"
                        : "hover:text-highlight-purple hover:scale-105"
                    }`}
                    onClick={() => setIsModalVisible(false)}
                  >
                    {section}
